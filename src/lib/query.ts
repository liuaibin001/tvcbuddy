...
export const useCodexStores = () => {
	return useSuspenseQuery({
		queryKey: ["codex-stores"],
		queryFn: () => invoke<CodexStore[]>("get_codex_stores"),
	});
};

export const useCreateCodexStore = () => {
	const queryClient = useQueryClient();
	...
	return useMutation({
		mutationFn: async ({
			...
			return invoke<CodexStore>("create_codex_store", { title, config });
		},
		onSuccess: () => {
			toast.success("Codex configuration created");
			queryClient.invalidateQueries({ queryKey: ["codex-stores"] });
		},
		onError: (error) => {
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			toast.error(`Failed to create Codex configuration: ${errorMessage}`);
		},
	});
};

export const useUpdateCodexStore = () => {
	const queryClient = useQueryClient();
	...
	return useMutation({
		mutationFn: ({
			...
			title: string;
			config: unknown;
		}) => invoke<CodexStore>("update_codex_store", { id, title, config }),
		onSuccess: () => {
			toast.success("Codex configuration saved");
			queryClient.invalidateQueries({ queryKey: ["codex-stores"] });
			queryClient.invalidateQueries({ queryKey: ["current-codex-store"] });
		},
		onError: (error) => {
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			toast.error(`Failed to save Codex configuration: ${errorMessage}`);
		},
	});
};

export const useDeleteCodexStore = () => {
	const queryClient = useQueryClient();

	return useMutation({
		mutationFn: (id: string) => invoke<void>("delete_codex_store", { id }),
		onSuccess: () => {
			toast.success("Codex configuration deleted");
			queryClient.invalidateQueries({ queryKey: ["codex-stores"] });
			queryClient.invalidateQueries({ queryKey: ["current-codex-store"] });
		},
		onError: (error) => {
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			toast.error(`Failed to delete Codex configuration: ${errorMessage}`);
		},
	});
};

export const useSetUsingCodexStore = () => {
	const queryClient = useQueryClient();

	return useMutation({
		mutationFn: (id: string) => invoke<void>("set_using_codex_store", { id }),
		onSuccess: () => {
			toast.success("Codex configuration activated");
			queryClient.invalidateQueries({ queryKey: ["codex-stores"] });
			queryClient.invalidateQueries({ queryKey: ["current-codex-store"] });
		},
		onError: (error) => {
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			toast.error(`Failed to activate Codex configuration: ${errorMessage}`);
		},
	});
};

export const useCurrentCodexStore = () => {
	return useQuery({
		queryKey: ["current-codex-store"],
		queryFn: () => invoke<CodexStore | null>("get_current_codex_store"),
	});
};

// Codex Global Settings hooks
...
export const useCodexGlobalSettings = () => {
	return useQuery({
		queryKey: ["codex-global-settings"],
		queryFn: () => invoke<CodexGlobalSettings>("get_codex_global_settings"),
	});
};

export const useUpdateCodexGlobalSettings = () => {
	const queryClient = useQueryClient();

	return useMutation({
		mutationFn: (settings: CodexGlobalSettings) =>
			invoke<void>("update_codex_global_settings", { settings }),
		onSuccess: () => {
			toast.success("Codex global settings updated");
			queryClient.invalidateQueries({ queryKey: ["codex-global-settings"] });
		},
		onError: (error) => {
			const errorMessage =
				error instanceof Error ? error.message : String(error);
